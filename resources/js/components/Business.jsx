...
const FeatureCard = ({ icon, title, content, index }) => (
    <div className= {`flex flex-row p-6 rounded-[20px] ${index !== features.length - 1 ? "mb-6" : "mb-0"} feature-card`}>
      <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} bg-sand`}>
        <img src={icon} alt="icon" className="w-[50%] h-[50%] object-contain" />
      </div>
      <div className="flex-1 flex flex-col ml-2">
        <h4 className="font-poppins font-semibold text-blueGreen text-[18px] leading-[24px] mb-1">
          {title}
        </h4>
        <p className="font-poppins font-normal text-primary text-[14px] leading-[24px] mb-1">
          {content}
        </p>
      </div>
    </div>
...
const Business = () => (
    <section id="features" className ={layout.section}>
     <div className={layout.sectionInfo}>
        <h2 className= {` text-5xl ${styles.heading2}`}  > AUTOMATED TRANSACTIONS & REAL TIME UPDATES</h2>
        <p  className= {`font font-poppins text-[15px]  text-primary xs:leading[26px]  max-w-[470px] mt-5`}>
